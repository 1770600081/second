...
require_once(dirname(dirname(__FILE__))."/vendor/AliPay/AlipayPay.php");
class Pay{
    public static function alipay($orderid)
    {
        $amount = ShopOrder::find()->where('orderid = :oid', [':oid' => $orderid])->one()->amount;
        if (!empty($amount)) {
            ...
            die;
            // $giftname = "慕课商城";
            // $data = ShopOrderDetail::find()->where('orderid = :oid', [':oid' => $orderid])->all();
            // $body = "";
            // foreach($data as $pro) {
            //     $body .= ShopProduct::find()->where('productid = :pid', [':pid' => $pro['productid']])->one()->title . " - ";
            // }
            // $body .= "等商品";
            // $showUrl = "basic.com";
