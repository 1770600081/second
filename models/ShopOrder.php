...
class ShopOrder extends ActiveRecord
{
    ...
    {
        $details = ShopOrderDetail::find()->where('orderid = :oid', [':oid' => $order->orderid])->all();
        $products = [];
        foreach($details as $detail) {
            $product = ShopProduct::find()->where('productid = :pid', [':pid' => $detail->productid])->one();
            if (empty($product)) {
                continue;
            }
        ...
        $order->address = ShopAddress::find()->where('addressid = :aid', [':aid' => $order->addressid])->one();
    ...
    public static function getProducts($userid)
    {
        ...
        foreach($orders as $order) {
            ...
            foreach($details as $detail) {
                ...
                $product->cate = ShopCategory::find()->where('cateid = :cid', [':cid' => $product->cateid])->one()->title;
                $products[] = $product;
            }
            $order->zhstatus = self::$status[$order->status];
            $order->products = $products;
